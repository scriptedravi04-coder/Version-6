...
async function startServer() {
  ...
  router.get("/verifications/me", (req, res) => {
    ...
    const list = db.verifications.filter((v) => v.user_id === actingId);
    if (list.length === 0) {
  ...
  router.post("/campaigns", (req, res) => {
    ...
    const bp = db.brand_profiles.find((p) => p.user_id === actingId);
    if (!bp || !bp.verified) {
      return res.status(403).json({ detail: "Please complete your KYC to post campaigns." });
    }
  ...
  router.post("/campaigns/apply", async (req, res) => {
    ...
    const cp = db.creator_profiles.find((p) => p.user_id === user.user_id);
    if (!cp || !cp.verified) {
      return res.status(403).json({ detail: "Please complete your KYC to apply for campaigns." });
    }

    const { campaign_id, proposed_amount, pitch } = req.body;
  ...
  router.post("/campaigns/:campaign_id/applications/:application_id/action", (req, res) => {
    ...
    if (action === "accept") {
      ...
      db.notifications.push({
        ...
        message: `Congratulations! Your proposal for '${camp.title}' has been accepted.`,
        read: false,
        created_at: getIsoNow(),
      });

      // Create a messaging/chat thread
      db.chat_threads = db.chat_threads || [];
  ...
  router.post("/chat/v2/threads/:threadId/offer", async (req, res) => {
    ...
    const db = getDb();
    const { threadId } = req.params;
    const { amount, deliverables, deadline, revision_count } = req.body;
  ...
  router.post("/campaigns/create", (req, res) => {
    ...
    const campaign = {
      ...
      title: req.body.title,
      description: req.body.description,
      budget_min: req.body.budget_min || 1000,
      budget_max: req.body.budget_max || 10000,
