...
export default function Landing() {
  ...
  return (
    <div className="text-foreground overflow-hidden" data-testid="landing-page">
      <Hero/>
      <StatsBar/>
  ...
  if (!events.length) {
    return (
      <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#7C5CFF]/10 border border-[#7C5CFF]/30 text-sm" data-testid="hero-live-pill">
        ...
        <span className="text-foreground/90">Loading active network events...</span>
      </span>
    );
  }
    ...
    <div className="h-12 flex items-center justify-center relative select-none max-w-full overflow-hidden">
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-[#7C5CFF]/12 border border-[#7C5CFF]/35 text-xs sm:text-sm shadow-[0_4px_12px_rgba(124,92,255,0.15)] max-w-[90vw] sm:max-w-xl text-center truncate"
        ...
        >
          ...
          <span className="text-foreground/95 font-medium tracking-normal font-sans text-xs sm:text-sm block truncate">
            {activeEvent.text}
          </span>
        </motion.span>
      </AnimatePresence>
  ...
  return (
    <section className="relative">
      ...
      <div className="relative max-w-6xl mx-auto px-6 md:px-10 pt-16 md:pt-24 pb-20 md:pb-28 text-center">
        ...
        <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.4,duration:0.6}} className="mt-10 italic text-lg md:text-xl text-foreground/65">
        ...
        <div className="max-w-6xl mx-auto px-6 md:px-10 py-10 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {items.map((s, i) => (
            <motion.div key={s.l} initial={{opacity:0,y:14}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.07, duration:0.4}}>
              ...
              <div className="text-sm text-foreground/55 mt-1">{s.l}</div>
            </motion.div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 md:px-10 py-24 md:py-28" data-testid="usps-section">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <p className="mt-5 text-foreground/55">Four platform-exclusive features that no other Indian influencer marketplace offers.</p>
      </div>
        ...
        {usps.map((u, i) => {
          ...
          return (
            <motion.div key={u.num} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.08}} className="card-dark min-h-[340px] flex flex-col" data-testid={`usp-${u.num.replace(/\s/g,'')}`}>
              ...
              <p className="text-sm text-foreground/55 mt-3 leading-relaxed">{u.d}</p>
            </motion.div>
          );
        })}
      </div>
...
function BrandsMarquee() {
  return (
    <section className="py-16 overflow-hidden border-t border-foreground/5" data-testid="brands-marquee">
      <p className="text-center label-mini text-foreground/40">Trusted by Brands Across India</p>
      <div className="mt-10 relative">
        <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-background to-transparent z-10 pointer-events-none"></div>
        <div className="absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-background to-transparent z-10 pointer-events-none"></div>
        <div className="flex marquee-track w-max">
          {[...BRANDS, ...BRANDS, ...BRANDS].map((b, i) => (
            <span key={i} className="font-display text-2xl md:text-3xl mx-8 text-foreground/35 hover:text-foreground/80 transition-colors flex-shrink-0">{b}</span>
          ))}
        </div>
      </div>
    </section>
...
function PerformanceRankShowcase() {
  const rows = [
    ...
    { rank: 3, name: "Anjali Singh", city: "Delhi", promised: "80K", delivered: "61K", tier: "SILVER", score: 62, color: "from-slate-400/15 to-slate-500/15", border: "border-foreground/15", text: "text-foreground/70" },
  ];
  ...
  return (
    <section className="max-w-5xl mx-auto px-6 md:px-10 py-24" data-testid="performance-showcase">
      <div className="text-center">
        ...
        <p className="text-foreground/55 mt-5 max-w-xl mx-auto">After every campaign closes, each creator receives an objective score. A leaderboard ranks all collaborators by ROI delivered.</p>
      </div>
      ...
      <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="card-dark mt-14 p-0 overflow-hidden">
        <div className="px-6 py-5 border-b border-foreground/10 flex items-center justify-between">
          ...
          <span className="text-xs text-foreground/45">5 Creators · Campaign Closed</span>
        </div>
        ...
        <div className="divide-y divide-white/5">
          {rows.map((r) => {
            ...
            return (
              <div key={r.rank} className={`px-6 py-5 flex flex-col sm:grid sm:grid-cols-12 items-start sm:items-center gap-4 sm:gap-3 ${r.rank===1 ? "bg-[#7C5CFF]/[0.06]" : ""}`}>
                <div className="flex items-center gap-3 sm:col-span-1">
                  <span className="text-2xl">{medalIcon}</span>
                  <span className="sm:hidden font-display text-sm font-bold text-foreground/40">Rank #{r.rank}</span>
                </div>
                
                <div className="sm:col-span-5 w-full">
                  <div className="font-semibold text-base sm:text-sm">{r.name}</div>
                  <div className="text-xs text-foreground/45">{r.city} · Swiggy Diwali</div>
                </div>
                ...
                <div className="flex justify-between sm:justify-start w-full sm:w-auto sm:col-span-2 gap-2 mt-1 sm:mt-0">
                  <div className="text-left sm:text-right w-1/2 sm:w-full">
                    <div className="text-xs text-foreground/45">Promised</div>
                  ...
                  <div className="text-right sm:text-right w-1/2 sm:w-full">
                    <div className="text-xs text-foreground/45">Delivered</div>
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 md:px-10 py-24" data-testid="how-it-works">
      <div className="text-center">
        ...
        <p className="text-foreground/55 mt-5">Three steps to your first successful influencer collaboration.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
        {steps.map((s, i) => (
          <motion.div key={s.num} initial={{opacity:0,y:24}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*0.1}} className="text-center px-4">
            <div className={`w-16 h-16 rounded-2xl bg-foreground/5 border border-foreground/10 mx-auto flex items-center justify-center ${s.color}`}>{s.icon}</div>
            <div className={`label-mini mt-6 ${s.color}`}>{s.num}</div>
            <h3 className="font-display text-2xl mt-2">{s.t}</h3>
            <p className="text-sm text-foreground/55 mt-4 leading-relaxed">{s.d}</p>
          </motion.div>
        ))}
      </div>
    </section>
    ...
    <section className="max-w-5xl mx-auto px-6 md:px-10 pb-24">
      <div className="relative card-elevated p-10 md:p-16 text-center overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="mt-5 text-foreground/60 max-w-xl mx-auto">Public rate cards. Verified data. Performance-tracked ROI. Built for serious creators and serious brands.</p>
...
function LandingFooter() {
  return (
    <footer className="relative border-t border-foreground/5 bg-background pt-20 pb-12 overflow-hidden z-20">
      ...
      <div className="absolute bottom-[-1.5rem] lg:bottom-[-2.5rem] left-1/2 -translate-x-1/2 w-full text-center pointer-events-none select-none z-0">
        <span className="font-display text-[15vw] font-black text-foreground/[0.03] uppercase tracking-[0.16em] leading-none select-none filter blur-[0.5px]">
          YBEX
        </span>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8 pb-16">
          ...
          <div className="md:col-span-4 flex flex-col items-start gap-4">
            ...
            <p className="text-xs text-foreground/50 leading-relaxed max-w-xs mt-2">
            ...
            <div className="flex items-center gap-3 mt-4">
              {[
                ...
                <a 
                  ...
                  className="w-8 h-8 rounded-full bg-foreground/5 border border-foreground/10 flex items-center justify-center text-white/60 hover:text-[#D9F111] hover:border-[#D9F111]/45 hover:bg-foreground/[0.08] transition-all transform hover:scale-105"
                >
                  {soc.icon}
                </a>
              ))}
          ...
          <div className="md:col-span-3 col-span-1">
            <h4 className="text-xs font-black text-foreground/90 uppercase tracking-widest mb-4 font-display">Explore Platform</h4>
            <ul className="space-y-3">
              <li><Link to="/explore" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Creator Discovery</Link></li>
              <li><Link to="/signup?role=brand" className="text-xs text-foreground/50 hover:text-foreground transition-colors">D2C Alliances</Link></li>
              <li><Link to="/ugc-orders" className="text-xs text-foreground/50 hover:text-foreground transition-colors flex items-center gap-1.5">UGC Campaign Briefs <span className="text-[8px] bg-[#D9F111]/15 text-[#D9F111] px-1.5 py-0.5 rounded font-black uppercase">Live</span></Link></li>
              <li><Link to="/explore" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Premium Rate Cards</Link></li>
            </ul>
          </div>

          {/* Network Options Column */}
          <div className="md:col-span-2 col-span-1">
            <h4 className="text-xs font-black text-foreground/90 uppercase tracking-widest mb-4 font-display">Our Network</h4>
            <ul className="space-y-3">
              <li><Link to="/signup?role=brand" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Talent Agencies</Link></li>
              <li><Link to="/signup?role=brand" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Brand Managers</Link></li>
              <li><Link to="/signup?role=creator" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Verified Creators</Link></li>
              <li><Link to="/explore" className="text-xs text-white/50 hover:text-white transition-colors font-semibold text-[#D9F111]">Barter Directory</Link></li>
            </ul>
          </div>

          {/* Resource & Safety Column */}
          <div className="md:col-span-3 col-span-1">
            <h4 className="text-xs font-black text-foreground/90 uppercase tracking-widest mb-4 font-display">Resources &amp; Safety</h4>
            <ul className="space-y-3">
              <li><Link to="/info/about" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Creator Handbooks</Link></li>
              <li><Link to="/info/careers" className="text-xs text-foreground/50 hover:text-foreground transition-colors flex items-center gap-1.5">Careers <span className="text-[7px] border border-blue-500/40 text-blue-400 px-1 py-0.5 rounded font-black uppercase leading-none">Join Us</span></Link></li>
              <li><Link to="/privacy-policy" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Escrow Protection</Link></li>
              <li><Link to="/info/terms" className="text-xs text-foreground/50 hover:text-foreground transition-colors">Terms of Use</Link></li>
            </ul>
          </div>
        ...
        <div className="border-t border-foreground/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-5 text-[10px] tracking-wider text-foreground/35 font-medium">
          <div className="uppercase">
            DATRUX SYSTEMS PVT. LIMITED &copy; 2026. ALL RIGHTS RESERVED.
          </div>

          <div className="flex items-center gap-2 select-none uppercase text-foreground/50 font-bold">
          ...
          <div className="flex items-center gap-4">
            <Link to="/privacy-policy" className="hover:text-foreground transition-colors uppercase">Privacy Rules</Link>
            <span>·</span>
            <a href="https://ybexmedia.com" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors uppercase font-bold text-[#D9F111]">YBEXMEDIA.COM</a>
          </div>
        </div>
      </div>
    </footer>
