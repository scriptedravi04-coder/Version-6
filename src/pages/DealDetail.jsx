...
export default function DealDetail() {
  ...
  return (
    <div className="w-full max-w-none px-4 sm:px-6 md:px-12 py-8 bg-[#09090e] text-white select-none">
      
      {/* Upper Navigation Row */}
      <div className="max-w-4xl mx-auto mb-6 flex items-center justify-between">
        <Link to="/collabs" className="text-sm text-white/60 hover:text-[#9D7CFF] flex items-center gap-1.5 font-medium transition-colors">
          <ArrowLeft size={14} /> Back to My Collabs
        </Link>
        <Link to="/chat" className="text-xs bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl text-white/85 flex items-center gap-1.5 font-bold transition-all">
          <MessageSquare size={13} className="text-[#9D7CFF]" /> Open Chat Workspace
        </Link>
      </div>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-[#131224]/50 border border-white/10 rounded-3xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-white/50 mt-1.5">
              Creator ID: <span className="text-[#9D7CFF] font-semibold">{deal.to_user_id}</span> | Contract: <span className="font-semibold text-white/85">{deal.collab_id}</span>
            </p>
          </div>
          <div className="text-left md:text-right bg-white/5 p-4 rounded-2xl border border-white/5 shrink-0">
            <span className="text-[10px] uppercase font-bold text-white/40 block">Escrow Protected Payout</span>
        ...
        <div className="bg-[#131224]/20 border border-white/5 rounded-2xl p-6">
          <div className="flex items-center justify-between flex-wrap gap-2 mb-4">
            <span className="text-xs font-bold uppercase text-white/40 tracking-wider">Campaign Deliverable Timeline Milestones</span>
            ...
            {stagesList.map((stepName, idx) => {
              ...
              return (
                <div key={idx} className="relative group text-left">
                  <div className={`h-1 rounded-full ${done ? "bg-emerald-400" : active ? "bg-[#7C5CFF]" : "bg-white/10"}`} />
                  <span className={`block text-[10px] mt-2 font-bold leading-tight ${done ? "text-emerald-400" : active ? "text-[#7C5CFF]" : "text-white/30"}`}>
                    {idx + 1}. {stepName}
                  </span>
                </div>
              );
        ...
        {(!deal.agreement_signed_creator || !deal.agreement_signed_brand) && (
          <div className="bg-amber-500/5 border border-amber-500/15 rounded-3xl p-6 text-left space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Pending Counterparty Contract Electronic Signature</h3>
                <p className="text-xs text-white/70 leading-relaxed mt-1 font-normal">
            ...
            <div className="bg-[#0e0e1a]/80 p-4 rounded-xl text-[11px] text-white/50 max-h-40 overflow-y-auto scroll-thin leading-relaxed space-y-2 border border-white/5">
            ...
            <div className="pt-2 flex items-center gap-4 flex-wrap text-xs">
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 font-bold">
                {deal.agreement_signed_brand ? <Check className="text-emerald-400" size={13} /> : <Clock className="text-amber-400" size={13} />}
                <span>Brand signature status: {deal.agreement_signed_brand ? "STAMPED" : "PENDING"}</span>
              </div>
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10 font-bold">
                {deal.agreement_signed_creator ? <Check className="text-emerald-400" size={13} /> : <Clock className="text-amber-400" size={13} />}
            ...
            {((isCreator && !deal.agreement_signed_creator) || (isBrand && !deal.agreement_signed_brand)) && (
              <div className="pt-2 flex gap-3 max-w-md">
                <input 
                  ...
                  className="flex-grow bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#7C5CFF] text-xs font-bold leading-normal" 
                  placeholder="Type Full Legal Name"
                />
                <button 
                  onClick={handleSign} 
                  disabled={signing}
                  className="bg-[#7C5CFF] hover:bg-[#6D3FFF] px-6 py-2.5 rounded-xl text-xs font-bold text-white transition-all shadow-md shrink-0 cursor-pointer"
                >
        ...
        {deal.agreement_signed_creator && deal.agreement_signed_brand && (deal.stage?.toUpperCase() === "ACTIVE" || deal.stage?.toUpperCase() === "NEGOTIATING") && (
          <div className="bg-[#131224]/30 border border-white/10 rounded-3xl p-6 sm:p-8 text-left space-y-4">
            ...
            {isCreator ? (
              <form onSubmit={handleSubmitDraft} className="space-y-4 max-w-2xl">
                <p className="text-xs text-white/60">Upload video preview drafts (YouTube live previews, Loom links, Drive, or Dropbox file storage) to receive brand draft approval.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-white/50 uppercase tracking-widest mb-1.5">Draft Link URL</label>
                    <input 
                      type="url" 
                      value={draftVideo} 
                      onChange={(e) => setDraftVideo(e.target.value)} 
                      name="draft_video_url"
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-[#7C5CFF]" 
                      placeholder="e.g. https://drive.google.com/..."
                      required
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-white/50 uppercase tracking-widest mb-1.5">Draft Caption Copy Proposal</label>
                    <input 
                      type="text" 
                      value={draftCaption} 
                ...
                <div>
                  <label className="block text-[10px] font-bold text-white/50 uppercase tracking-widest mb-1.5">Notes to reviewers</label>
                  <textarea 
                    rows={3} 
                    value={draftNotes} 
                    onChange={(e) => setDraftNotes(e.target.value)} 
                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-xs focus:outline-none filter-none"
              ...
              <div className="text-center py-6 text-white/40">
                <Clock size={28} className="mx-auto mb-2 text-white/30" />
                <p className="text-xs font-semibold">Waiting for the creator to dispatch their proposed draft review asset link.</p>
                <p className="text-[10px] text-white/30 mt-1 font-normal">Funds remain securely protected by Escrow limits. Direct notifications will trigger once drafts register.</p>
              </div>
            )}
          </div>
        )}
        ...
        {deal.stage?.toUpperCase() === "CONTENT_SUBMITTED" && (
          <div className="bg-[#131224]/30 border border-white/10 rounded-3xl p-6 sm:p-8 text-left space-y-4">
            ...
            {deal.submission && (
              <div className="bg-[#0e0e1a]/80 p-5 rounded-2xl border border-white/5 space-y-3">
                <div>
                  ...
                  <a 
                    ...
                    className="text-xs text-white underline font-semibold flex items-center gap-1.5 mt-1 hover:text-[#9D7CFF] transition-colors"
                  >
                ...
                {deal.submission.caption && (
                  <div>
                    <span className="text-[10px] text-white/40 font-bold uppercase block tracking-wider">Proposed Caption Copy</span>
                    <p className="text-xs text-white/90 mt-1 font-normal select-text">&ldquo;{deal.submission.caption}&rdquo;</p>
                  </div>
                )}
                {deal.submission.notes && (
                  <div>
                    <span className="text-[10px] text-white/40 font-bold uppercase block tracking-wider">Creator notes</span>
                    <p className="text-xs text-white/70 mt-1 italic font-normal">&ldquo;{deal.submission.notes}&rdquo;</p>
                  </div>
                )}
              </div>
            )}
            ...
            {isBrand ? (
              <div className="pt-2 space-y-4">
                ...
                <div className="border-t border-white/5 pt-4 max-w-md">
                  <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest block mb-1.5">Request modifications / Revisions</span>
                  <div className="flex gap-2">
                    <input 
                      type="text" 
                      value={revisionFeedback} 
                      onChange={(e) => setRevisionFeedback(e.target.value)} 
                      className="flex-grow bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none" 
                      placeholder="e.g. Please trim first 5 seconds of footage..."
                    />
                    <button 
                      onClick={handleRequestChanges} 
                      disabled={rejecting} 
                      className="bg-[#7C5CFF]/10 hover:bg-[#7C5CFF]/20 text-white border border-[#7C5CFF]/20 px-4 rounded-xl text-xs font-bold transition-colors cursor-pointer"
                    >
                      Request Revision
                    </button>
                  </div>
              ...
              <div className="bg-amber-500/5 border border-amber-500/15 p-5 rounded-xl text-center">
                ...
                <p className="text-[10px] text-white/60 mt-1">The matching Brand brand reviewers have been notified. Revision suggestions or direct permissions will trigger shortly.</p>
              </div>
            )}
          </div>
        )}
        ...
        {deal.stage?.toUpperCase() === "CONTENT_APPROVED" && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-6 sm:p-8 text-center space-y-4">
            <h3 className="text-lg font-display font-black text-emerald-400">Content Pre-Release Approved!</h3>
        ...
        {(deal.stage?.toUpperCase() === "PROOF_SUBMITTED" || deal.stage?.toUpperCase() === "COMPLETED") && (
          <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-3xl p-6 sm:p-8 text-center space-y-4">
            ...
            <p className="text-xs text-white/80 max-w-lg mx-auto leading-relaxed font-normal">
            ...
            <div className="pt-2">
              <button 
                ...
                className="bg-[#7C5CFF] hover:bg-[#6D3FFF] text-white font-extrabold text-xs px-8 py-4 rounded-xl transition-all shadow-lg inline-flex items-center gap-1.5 cursor-pointer"
              >
