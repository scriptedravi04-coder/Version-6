  ...
  return (
    <div className="max-w-[1600px] w-full mx-auto px-4 lg:px-8 py-8 lg:py-12 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 relative z-10">
        <div>
          <h1 className="text-3xl lg:text-4xl font-display font-bold text-white tracking-tight">My UGC Briefs</h1>
          <p className="text-white/50 mt-2 font-medium">Manage your active product UGC requests</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 relative z-10">
        {[
          ...
          <div key={s.label} className="bg-[#12121A] border border-white/5 shadow-xl rounded-[2rem] p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 relative z-10">
        {briefs.length === 0 ? (
          <div className="col-span-full text-center py-32 bg-[#0A0A0F] border border-white/10 rounded-[2.5rem] shadow-2xl">
            <h3 className="text-xl font-bold text-white mb-2">No active briefs</h3>
            <p className="text-white/50 mb-8 font-medium">Post a brief to get original creator videos within 24 hours.</p>
            <button onClick={() => navigate("/brand/ugc/post")} className="bg-white text-black font-bold px-8 py-3.5 rounded-full text-sm hover:bg-gray-200 transition-colors shadow-lg active:scale-95">Create Brief</button>
          </div>
        ) : briefs.map(b => (
                 ...
                 <div className="bg-[#1A1A24] p-6 relative overflow-hidden flex-shrink-0 border-b border-white/[0.02]">
                   ...
                   <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex flex-col items-end">
                         ...
                         <span className="text-base font-black text-white bg-black/40 px-3 py-1 rounded-xl border border-white/5 shadow-inner">₹{b.budget?.toLocaleString()}</span>
                      </div>
                   </div>
                 ...
                 <div className="bg-[#0A0A0F] p-6 flex-1 flex flex-col justify-end">
                   <div>
                      ...
                      <div className="flex gap-3 pt-6 border-t border-white/5">
                         <button onClick={() => navigate("/brand/ugc/orders")} className="flex-1 bg-white/5 hover:bg-white/10 active:bg-white/15 border border-white/10 text-white text-xs font-bold py-3 rounded-xl transition-colors">Orders</button>
                         <button className="flex-1 bg-[#1A1A24] hover:bg-[#252530] text-white/50 hover:text-white/80 text-xs font-bold py-3 rounded-xl transition-colors border border-transparent">Pause</button>
                      </div>
                   </div>
                 </div>
              </div>
