...
import { X, CheckCircle, Ban, HelpCircle, Briefcase, FileText, Send, Sparkles } from "lucide-react";
...
export default function Collabs() {
  ...
  const [tab, setTab] = useState("campaign_applications");
  ...
  return (
    <div className="w-full max-w-none px-4 md:px-8 py-10 relative" data-testid="collabs-page">
      <h1 className="font-display text-3xl sm:text-4xl md:text-5xl tracking-tight text-white mb-2">Collaborations & Proposals</h1>
      <p className="text-white/60 text-sm">Review, negotiate, and process contracts, applications, and creator waves.</p>
        ...
        {tabs.map(t => (
          <button 
            ...
            className={`px-4 py-3 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors flex items-center gap-2 ${
              tab === t.id ? "border-collabs-tab border-[#7C5CFF] text-[#9D7CFF]" : "border-transparent text-white/50 hover:text-white"
            }`}
          >
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${
              tab === t.id ? "bg-[#7C5CFF]/30 text-white" : "bg-white/5 text-white/50"
            }`}>
              {t.count}
            </span>
          </button>
      ...
      <div className="space-y-4 mt-6">
        {items.length === 0 && (
          <div className="bg-[#0e0e1a]/80 border border-white/5 rounded-2xl p-16 text-left text-white/40 max-w-xl ml-0 flex flex-col items-start justify-start">
        ...
        {tab !== "campaign_applications" && items.map((it, i) => (
          <div key={it.collab_id || it.wave_id || i} className="bg-card border border-foreground/10 rounded-2xl p-5 flex items-center justify-between gap-4 flex-wrap sm:flex-nowrap hover:border-white/10 transition-all">
            <div>
              <div className="font-display text-lg text-white font-medium">{it.from_name || it.to_user_id}</div>
              {it.deliverable && <div className="text-sm text-foreground/70">{it.deliverable} · ₹{(it.proposed_amount || 0).toLocaleString("en-IN")}</div>}
              {it.message && <div className="text-sm text-foreground/50 mt-1 italic leading-relaxed">&ldquo;{it.message}&rdquo;</div>}
              <div className="text-[10px] text-foreground/40 mt-1.5 font-mono">{new Date(it.created_at).toLocaleString()}</div>
            </div>
            <div className="flex items-center gap-2">
              {it.status && (
                <span className={`text-xs px-2 py-1 rounded-full whitespace-nowrap font-mono font-semibold uppercase tracking-wider ${
                  ...
                  it.status === "accepted" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/15" : 
              ...
              {tab === "received" && it.status === "pending" && (
                <>
                  ...
                  <button onClick={() => act(it.collab_id, "decline")} data-testid={`decline-${it.collab_id}`} className="px-3 py-1.5 border border-foreground/10 text-xs font-semibold hover:bg-card/5 whitespace-nowrap rounded-lg text-foreground/85 transition-all">Decline</button>
                </>
              )}
              {it.status === "accepted" && (
                <Link 
                  to={`/deals/${it.collab_id}`} 
                  className="px-3 py-1.5 bg-[#7C5CFF]/20 hover:bg-[#7C5CFF]/35 text-white text-xs font-bold border border-[#7C5CFF]/35 rounded-lg shadow-sm transition-all whitespace-nowrap"
                >
                  Track progress &rarr;
                </Link>
              )}
              {tab === "sent" && it.status === "accepted" && (
                <button 
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-[#7C5CFF] to-[#9D7CFF] hover:opacity-90 text-white text-xs font-bold rounded-lg shadow transition-opacity whitespace-nowrap uppercase tracking-wider"
                >
                  Pay Now
                </button>
              )}
        ...
        {tab === "campaign_applications" && items.map((it, i) => (
          <div key={it.application_id || it.campaign_id || i} className="bg-card border border-foreground/10 rounded-2xl p-6 flex items-center justify-between gap-5 flex-wrap sm:flex-nowrap hover:border-white/10 transition-all animate-in fade-in duration-200">
            <div className="space-y-1 pl-1">
              <div className="font-display text-lg text-white font-semibold">
                {user?.role === "brand" ? it.creator_name : it.brand_name}
              </div>
              <div className="text-xs text-[#9D7CFF] font-semibold font-mono flex items-center gap-1">
                <span>Campaign:</span>
                <span className="underline">{it.campaign_title}</span>
              </div>
              {it.pitch && (
                <p className="text-sm text-white/70 max-w-2xl bg-white/5 px-3 py-2 rounded-xl mt-2 border border-white/5 leading-relaxed italic">
                  &ldquo;{it.pitch}&rdquo;
                </p>
              )}
              <div className="text-xs text-white/55 font-semibold pt-2">
                Proposed Compensation: <span className="text-emerald-400 font-mono text-sm font-bold">₹{(it.proposed_amount || 0).toLocaleString("en-IN")}</span>
              </div>
              <div className="text-[10px] text-white/35 pt-1 font-mono">{new Date(it.applied_at).toLocaleString()}</div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              <span className={`text-[10px] px-2 py-1 rounded-full font-mono font-bold uppercase tracking-wider border ${
                ...
                it.status === "accepted" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/25" : 
              ...
              {user?.role === "brand" && it.status === "pending" && (
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="px-3 py-1.5 border border-white/10 hover:bg-white/5 text-white/80 text-xs font-bold rounded-lg transition-colors flex items-center gap-1"
                  >
                    <Ban size={13} /> Decline
                  </button>
                </div>
      ...
      {payingCollab && (
         <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
           <div className="relative w-full max-w-lg">
             <button onClick={() => setPayingCollab(null)} className="absolute -top-10 right-4 p-2 text-white/50 hover:text-white transition-colors">
               <X size={24} />
             </button>
