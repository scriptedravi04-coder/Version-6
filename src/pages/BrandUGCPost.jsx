...
export default function BrandUGCPost() {
  ...
  const Step1 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div>
        ...
        <input type="text" className="w-full bg-[#0f0f1a] border border-white/10 rounded-xl px-4 py-4 text-white focus:border-[#7c3aed] transition-colors outline-none" placeholder="e.g. Try on haul for activewear" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
      </div>
      <div>
        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Detailed Requirements</label>
        <textarea rows={6} className="w-full bg-[#0f0f1a] border border-white/10 rounded-xl px-4 py-4 text-white focus:border-[#7c3aed] transition-colors outline-none" placeholder="Provide full details about exactly what you expect in the video..." value={formData.detailed_requirements || ''} onChange={e => setFormData({...formData, detailed_requirements: e.target.value})} />
      </div>
      <button onClick={() => setStep(2)} className="w-full bg-white hover:bg-gray-100 text-black font-bold py-4 rounded-xl mt-4 active:scale-95 transition-all shadow-xl">Next: Product Info</button>
    </div>
  );
  ...
  const Step2 = () => (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4">
      <div>
        ...
        <input type="text" className="w-full bg-[#0f0f1a] border border-white/10 rounded-xl px-4 py-3.5 text-white focus:border-[#7c3aed] transition-colors outline-none" placeholder="e.g. Pro Whey Protein" value={formData.product_name} onChange={e => setFormData({...formData, product_name: e.target.value})} />
      </div>
      <div>
        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Sample Content URL (Optional)</label>
        <input type="url" className="w-full bg-[#0f0f1a] border border-white/10 rounded-xl px-4 py-3.5 text-white focus:border-[#7c3aed] transition-colors outline-none" placeholder="Link to a similar video or moodboard" value={formData.sample_content_url || ''} onChange={e => setFormData({...formData, sample_content_url: e.target.value})} />
      </div>
      <div>
      ...
      <div>
        ...
        <input type="url" className="w-full bg-[#0f0f1a] border border-white/10 rounded-xl px-4 py-3.5 text-white focus:border-[#7c3aed] transition-colors outline-none" placeholder="https://" value={formData.product_url} onChange={e => setFormData({...formData, product_url: e.target.value})} />
      </div>
      <div className="flex gap-3 pt-2">
         <button onClick={() => setStep(1)} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-3.5 rounded-xl active:scale-95 transition-all mb-4 border border-white/10">Back</button>
         <button onClick={() => setStep(3)} className="w-2/3 bg-white hover:bg-gray-100 text-black font-bold py-3.5 rounded-xl active:scale-95 transition-all mb-4 shadow-xl">Next: Deliverables</button>
      </div>
    </div>
  );
  ...
  const Step3 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {['instagram_reel', 'instagram_story', 'youtube_short', 'ugc_raw_video'].map(type => (
            <button key={type} onClick={() => setFormData({...formData, deliverable_type: type})} className={`px-4 py-3 rounded-xl border text-[11px] font-bold uppercase tracking-wider transition-all ${formData.deliverable_type === type ? 'border-[#7c3aed] bg-[#7c3aed]/10 text-white shadow-[0_0_15px_rgba(124,58,237,0.3)]' : 'border-white/10 text-gray-500 bg-[#0f0f1a] hover:border-white/20'}`}>
              {type.replace(/_/g," ")}
            </button>
          ))}
        </div>
      ...
      <div className="flex gap-3 pt-4">
         <button onClick={() => setStep(2)} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-3.5 rounded-xl active:scale-95 transition-all border border-white/10">Back</button>
         <button onClick={() => setStep(4)} className="w-2/3 bg-white hover:bg-gray-100 text-black font-bold py-3.5 rounded-xl active:scale-95 transition-all shadow-xl">Next: Budget</button>
      </div>
    </div>
  );
  ...
  const Step4 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="pt-4">
        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Number of Creators Needed</label>
         <div className="flex gap-2">
            {[1,2,3,5,10].map(num => (
              <button key={num} onClick={() => setFormData({...formData, max_creators: num})} className={`w-12 h-12 rounded-xl border font-bold transition-all ${formData.max_creators === num ? 'border-[#7c3aed] bg-[#7c3aed]/10 text-white shadow-[0_0_15px_rgba(124,58,237,0.3)]' : 'border-white/10 text-gray-500 bg-[#0f0f1a] hover:border-white/20'}`}>
                {num}
              </button>
            ))}
         </div>
      ...
      <div className="flex gap-3 pt-8">
         <button onClick={() => setStep(3)} className="flex-1 bg-white/5 hover:bg-white/10 text-white font-bold py-3.5 rounded-xl border border-white/10 active:scale-95 transition-all">Back</button>
         <button onClick={() => setStep(5)} className="w-2/3 bg-white hover:bg-gray-100 text-black font-bold py-3.5 rounded-xl active:scale-95 transition-all shadow-xl">Review & Pay</button>
      </div>
    </div>
  );
  ...
  const Step5 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="bg-[#16161e] rounded-3xl p-6 border border-[#27273a] shadow-xl">
      ...
      <div className="flex gap-3">
         <button disabled={isSubmitting} onClick={() => setStep(4)} className="bg-white/5 hover:bg-white/10 text-white font-bold py-3.5 px-6 rounded-xl border border-white/10 active:scale-95 transition-all disabled:opacity-50">Back</button>
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 flex flex-col justify-center relative">
      ...
      <div className="w-full flex-col flex h-full justify-center max-w-xl mx-auto lg:mx-0">
        <h1 className="text-3xl font-display font-bold text-white mb-8 tracking-tight">Create New Campaign</h1>
        {/* Stepper Header */}
        <div className="flex items-center gap-2 w-full mb-10 overflow-hidden">
          {[1,2,3,4,5].map(num => (
            <React.Fragment key={num}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors shadow-lg ${step === num ? 'bg-[#7c3aed] text-white' : step > num ? 'bg-white/20 text-white' : 'bg-white/5 text-gray-500 border border-white/10'}`}>
                {step > num ? <Check size={14}/> : num}
              </div>
              {num < 5 && <div className={`h-1 flex-1 rounded-full bg-white/5 ${step > num ? 'bg-white/20' : ''}`} />}
            </React.Fragment>
          ))}
        </div>
      ...
      <div className="w-full hidden lg:flex justify-center xl:justify-end items-center">
         <div className="w-full max-w-[360px]">
           ...
           <div className="w-full rounded-[2.5rem] overflow-hidden relative shadow-2xl bg-[#0A0A0F] border border-white/10 aspect-[9/16]">
              ...
              <div className="absolute inset-0 bg-[#16161e] opacity-90 -z-10 text-white"></div>
              
              {/* Centered Content */}
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10">
                 <Video size={48} className="mx-auto mb-4 text-white/30 drop-shadow-xl" strokeWidth={1.5} />
                 <h3 className="text-2xl font-bold text-white mb-3 tracking-tight">{formData.title || `Review of ${formData.product_name || 'Product'}`}</h3>
                 <p className="text-white/60 text-sm font-medium line-clamp-3 px-4">{formData.product_description || 'Product description goes here'}</p>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-black/90 via-black/50 to-transparent z-20">
                 <div className="flex flex-wrap items-center gap-2 mb-4">
                    <div className="bg-[#7c3aed] text-white px-3.5 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-[0_0_15px_rgba(124,58,237,0.4)]">
                       {formData.deliverable_type?.replace(/_/g," ") || 'FORMAT'}
                    </div>
                    <div className="bg-white/20 backdrop-blur-md text-white px-3.5 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                       {formData.video_duration || 'DURATION'}
                    </div>
                 </div>
                 <div className="flex justify-between items-end mt-2 border-t border-white/10 pt-4">
                    <div className="text-white">
                      <p className="text-[10px] text-white/50 font-bold uppercase tracking-widest mb-1">Budget</p>
