  ...
  const loadData = async () => {
    try {
      ...
      const totalSpent = allDeals
        ...
        .reduce((sum, current) => sum + Number(current.proposed_amount || current.budget || 0), 0) || 37000;

      const totalApps = camps.reduce((sum, c) => sum + (c.applicants?.length || 0), 0) || 12;

      setStats({
        campaignsCount: camps.length || 3,
        applicantsCount: totalApps,
        dealsCount: activeDeals || 2,
