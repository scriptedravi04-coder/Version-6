...
export default function ApplicantCard({ applicant, onShortlist, onReject, onViewProfile }) {
  ...
  return (
    <div className="bg-[#131224]/80 border border-white/5 p-5 rounded-2xl shadow-sm text-left relative overflow-hidden hover:border-[#7C5CFF]/20 transition-all duration-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="text-base font-bold text-white leading-tight">{a.full_name || "Ravi Kumar"}</h4>
              <span className="text-xs font-mono font-bold text-[#9D7CFF]">{followersCount} followers</span>
            </div>
            <div className="text-xs text-white/50 mt-1 flex items-center gap-1.5 font-sans">
        ...
        {/* Commercial summary */}
        <div className="bg-white/[0.02] border border-white/5 px-4 py-2.5 rounded-xl text-left sm:text-right font-mono min-w-[140px]">
          <div className="text-[10px] text-white/40 uppercase">Proposed Fee</div>
          <div className="text-sm font-bold text-emerald-400 mt-0.5">{rateProposed}</div>
          <div className="text-[9px] text-white/50 mt-1">Delivery: {estDelivery}</div>
        </div>
      </div>

      {/* Pitch pitch */}
      <div className="mt-4 bg-white/[0.01] border-l border-[#7C5CFF]/30 pl-3.5 py-1 text-xs text-white/70 leading-relaxed italic">
        &ldquo;{bio}&rdquo;
      </div>
      ...
      <div className="mt-5 pt-4 border-t border-white/5 flex flex-wrap items-center justify-between gap-3">
        <button
          ...
          className="px-3.5 py-1.5 rounded-lg text-xs font-bold border border-white/10 hover:bg-white/5 text-white/70 transition-all cursor-pointer flex items-center gap-1.5"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg text-xs font-bold bg-[#7C5CFF] hover:bg-[#6B4AFF] text-white shadow-md transition-all cursor-pointer flex items-center gap-1.5"
          >
