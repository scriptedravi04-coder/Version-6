  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         <div className="bg-card border border-foreground/10 rounded-2xl p-5 flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center">
              <Users size={24} />
            </div>
            <div>
              <div className="text-3xl font-display font-bold">{totalWaitlisted}</div>
              <div className="text-sm text-foreground/50">Total Pending Waitlist</div>
            </div>
         </div>
         <div className="bg-card border border-foreground/10 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-foreground/50">Approved Today</div>
            </div>
         </div>
         <div className="bg-card border border-foreground/10 rounded-2xl p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-foreground/50">Rejected Today</div>
            </div>
         </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-black/40 border border-foreground/10 rounded-xl focus:border-[#9D7CFF] focus:outline-none text-sm"
            />
          </div>
          <div className="relative">
            <select 
              ...
              className="appearance-none pl-4 pr-10 py-2.5 bg-black/40 border border-foreground/10 rounded-xl focus:border-[#9D7CFF] focus:outline-none text-sm"
            >
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center gap-3">
            <span className="text-sm text-foreground/60">{selectedIds.length} selected</span>
              ...
              {filteredWaitlist.map(user => (
                ...
                >
                  <td className="px-4 py-4 truncate">
                    {user.status === 'Pending' ? (
                      <button onClick={(e) => toggleSelect(e, user.id)} className="p-1 hover:text-foreground text-foreground/40">
                  ...
                  <td className="px-4 py-4 font-mono text-foreground/60">#{user.position}</td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      <img src={user.photo} alt={user.name} className="w-10 h-10 rounded-full border border-foreground/10" />
                      <div>
                        <div className="font-semibold">{user.name}</div>
                        <div className="text-xs text-foreground/50">{user.handle}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                       <span className="text-foreground/80">{user.platform}</span>
                  ...
                  <td className="px-4 py-4 text-foreground/60">{user.date}</td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-4 py-4 text-right">
                    {user.status === 'Pending' && (
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={(e) => handleRejectAction(e, user.id)} className="px-3 py-1.5 bg-foreground/5 hover:bg-red-500/20 text-foreground/70 hover:text-red-400 rounded-lg text-xs font-medium transition-colors">
                          Reject
                        </button>
                      </div>
                    )}
                    {user.status === 'Rejected' && (
                      <div className="text-xs text-foreground/40 max-w-[120px] truncate ml-auto" title={user.rejectReason}>
      ...
      {rejectModalId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#12121A] border border-foreground/10 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
            ...
            <div className="p-5">
              <label className="block text-sm font-medium text-foreground/70 mb-2">Reason for rejection *</label>
              <div className="relative">
                <select 
                  value={rejectReason}
                  onChange={(e) => setRejectReason(e.target.value)}
                  className="w-full appearance-none pl-4 pr-10 py-3 bg-black/40 border border-foreground/10 rounded-xl focus:border-red-500 focus:outline-none"
                >
            ...
            <div className="p-5 bg-black/20 flex gap-3 justify-end items-center">
              <button onClick={() => {setRejectModalId(null); setRejectReason("");}} className="px-4 py-2 text-foreground/60 hover:text-foreground text-sm font-medium">Cancel</button>
              <button 
                ...
                className="px-4 py-2 bg-red-500 text-white rounded-xl text-sm font-semibold disabled:opacity-50 transition-colors"
              >
                Confirm Rejection
              </button>
            </div>
