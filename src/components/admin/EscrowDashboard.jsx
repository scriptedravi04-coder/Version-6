    ...
    <div className="space-y-6">
       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-card border border-foreground/10 rounded-2xl p-5">
             <div className="text-sm font-medium text-foreground/50 mb-1 flex items-center gap-2"><CreditCard size={16}/> Total Funds in Escrow</div>
          ...
          <div className="bg-card border border-foreground/10 rounded-2xl p-5">
             <div className="text-sm font-medium text-foreground/50 mb-1 flex items-center gap-2"><TrendingUp size={16}/> Platform Fees Collected</div>
          ...
          <div className="bg-card border border-foreground/10 rounded-2xl p-5">
             <div className="text-sm font-medium text-foreground/50 mb-1 flex items-center gap-2"><FileText size={16}/> GST Reported</div>
             <div className="font-display text-4xl font-bold">₹7.6K</div>
             <div className="text-xs text-foreground/40 mt-2 flex items-center gap-1">Liability this month</div>
          </div>
          <div className="bg-card border border-foreground/10 rounded-2xl p-5">
             <div className="text-sm font-medium text-foreground/50 mb-1 flex items-center gap-2"><ArrowUpRight size={16}/> Processed Payouts</div>
             <div className="font-display text-4xl font-bold">₹184K</div>
             <div className="text-xs text-foreground/40 mt-2 flex items-center gap-1">To creators (YTD)</div>
          </div>
       </div>
          ...
          <div className="p-4 border-b border-foreground/10 flex items-center justify-between">
             ...
             <div className="flex gap-2">
                <div className="relative">
                   ...
                   <input type="text" placeholder="Search TxID or account..." className="pl-9 pr-4 py-2 bg-black/40 border border-foreground/10 rounded-lg text-sm focus:border-[#9D7CFF] focus:outline-none"/>
                </div>
                <button className="flex items-center gap-2 px-4 py-2 bg-black/40 border border-foreground/10 rounded-lg text-sm font-semibold hover:bg-foreground/5 transition-colors">
               ...
               {transactions.map(tx => (
                  <tr key={tx.id} className="hover:bg-foreground/5 transition-colors">
                     <td className="px-4 py-4">
                        <div className="font-mono text-xs text-foreground/50">{tx.id}</div>
                        <div className="font-semibold mt-0.5 flex items-center gap-1.5">
                           ...
                           {tx.type === 'Escrow Lock' && <CreditCard size={14} className="text-foreground/60"/>}
                           {tx.type}
                        </div>
                        <div className="text-xs text-foreground/40 mt-0.5">{tx.date}</div>
                     </td>
                     <td className="px-4 py-4">
                        <div className="text-foreground/80"><span className="text-foreground/40 text-xs inline-block w-8">From:</span> {tx.from}</div>
                        <div className="text-foreground/80 mt-1"><span className="text-foreground/40 text-xs inline-block w-8">To:</span> {tx.to}</div>
                     </td>
                     ...
                     <td className="px-4 py-4 text-right">
                        ...
                        <div className="text-xs text-foreground/50">GST: ₹{tx.gst.toLocaleString()}</div>
                     </td>
