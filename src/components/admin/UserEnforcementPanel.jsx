...
export default function UserEnforcementPanel({ user, onBack, onUserUpdate }) {
  ...
  return (
    <div className="w-full max-w-5xl space-y-6">
       <button onClick={onBack} className="flex items-center gap-2 text-sm text-foreground/60 hover:text-foreground mb-4">
          <ArrowLeft size={16} /> Back to Users
       </button>

       <div className="bg-card border border-foreground/10 rounded-2xl p-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
             ...
             <div>
                ...
                <div className="text-sm text-foreground/50">{user.email} • {user.role}</div>
             </div>
          </div>
          {user.banned && (
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-6">
             {activeStage === 'warning' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                   ...
                   <div>
                      <label className="text-xs text-foreground/50 mb-1 block">Template</label>
                      <select value={warningTemplate} onChange={e=>setWarningTemplate(e.target.value)} className="w-full bg-black/40 border border-foreground/10 rounded-xl p-2.5 text-sm focus:outline-none focus:border-amber-500">
                   ...
                   <div>
                      <label className="text-xs text-foreground/50 mb-1 block">Custom Note</label>
                      <textarea value={warningNote} onChange={e=>setWarningNote(e.target.value)} className="w-full h-20 bg-black/40 border border-foreground/10 rounded-xl p-2.5 text-sm focus:outline-none focus:border-amber-500 resize-none"></textarea>
                   </div>
                   <button 
             ...
             {activeStage === 'suspend' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                   ...
                   <div>
                      <label className="text-xs text-foreground/50 mb-1 block">Duration</label>
                      <select value={suspendDuration} onChange={e=>setSuspendDuration(e.target.value)} className="w-full bg-black/40 border border-foreground/10 rounded-xl p-2.5 text-sm focus:outline-none focus:border-orange-500">
                   ...
                   <div>
                      <label className="text-xs text-foreground/50 mb-1 block">Reason</label>
                      <select value={suspendReason} onChange={e=>setSuspendReason(e.target.value)} className="w-full bg-black/40 border border-foreground/10 rounded-xl p-2.5 text-sm focus:outline-none focus:border-orange-500">
                   ...
                   <button 
                      ...
                      className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50">
             ...
             {activeStage === 'ban' && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                   ...
                   <p className="text-xs text-foreground/50">This action is irreversible. The user will be entirely blocked and all active sessions will be terminated.</p>
                   <div>
                      <label className="text-xs text-foreground/50 mb-1 block">Type username to confirm</label>
                      <input 
                         type="text" 
                         value={banConfirmText} 
                         onChange={e=>setBanConfirmText(e.target.value)} 
                         placeholder={user.name}
                         className="w-full bg-black/40 border border-red-500/30 rounded-xl p-2.5 text-sm focus:outline-none focus:border-red-500"
                      />
                   </div>
                   <button 
                      disabled={banConfirmText !== user.name}
                      onClick={() => { handleAction('ban', {}); setBanConfirmText(''); }}
                      className="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-colors disabled:opacity-50"
                   >
          ...
          <div className="space-y-6">
             ...
             <div className="bg-card border border-foreground/10 rounded-2xl p-6">
                <h4 className="font-semibold text-sm mb-3 text-foreground/60">Notification Preview</h4>
                <div className="p-4 bg-black/40 border border-foreground/5 rounded-xl text-sm italic text-foreground/80 leading-relaxed">
                   "{getPreviewText()}"
                </div>
             </div>
                   ...
                   <div className="space-y-4">
                      {violations.map(v => (
                         <div key={v.id} className="flex gap-3 text-sm">
                            ...
                            <div>
                               <div className="font-medium">{v.type} <span className="text-xs text-foreground/50 font-normal ml-2">{v.date}</span></div>
                               <div className="text-foreground/70 mt-0.5">{v.reason}</div>
                            </div>
                         </div>
                      ))}
                   </div>
                ) : (
                   <p className="text-sm text-foreground/50 italic">No past violations recorded.</p>
                )}
             </div>
...
function StageButton({ active, icon, label, color, bgColor, onClick }) {
   return (
      <button onClick={onClick} className="flex flex-col items-center gap-2 z-10 group bg-card transition-transform">
         <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all ${active ? `${bgColor} ${color} scale-110` : 'bg-card border-foreground/10 text-foreground/40 group-hover:border-foreground/30'}`}>
            {icon}
         </div>
         <span className={`text-xs font-semibold ${active ? color : 'text-foreground/50'}`}>{label}</span>
      </button>
   );
}
