...
export default function CampaignReviewQueue() {
  ...
  const filtered = campaigns.filter(c => (c.stage || c.status) === filter);

  const stats = {
    'Draft': campaigns.filter(c => (c.stage || c.status) === 'Draft').length,
    'Under Review': campaigns.filter(c => (c.stage || c.status) === 'Under Review').length,
    'Live': campaigns.filter(c => (c.stage || c.status) === 'Live').length,
    'Rejected': campaigns.filter(c => (c.stage || c.status) === 'Rejected').length,
  };
