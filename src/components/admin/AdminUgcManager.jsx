...
export default function AdminUgcManager() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading UGC Orders...</div>;

  return (
    <div className="space-y-6">
       <div className="bg-[#16161e] rounded-3xl p-6 border border-white/10">
         <h2 className="text-xl font-bold mb-4">UGC Orders Management</h2>
         <p className="text-sm text-gray-500 mb-6">Track delayed delivery and upload via in-house team if creator misses deadline.</p>

         {orders.length === 0 ? (
           <div className="text-sm text-gray-400 font-medium">Use Creator dashboard to generate orders in demo mode to test this feature.</div>
         ) : (
           <table className="w-full text-left text-sm">
             <thead className="text-[10px] uppercase font-bold text-gray-500 tracking-widest border-b border-white/10">
               <tr>
             ...
             <tbody className="divide-y divide-white/5">
                {orders.map(o => (
                  <tr key={o.id}>
                    ...
                    <td className="py-4">
                      <button onClick={() => handleTeamUpload(o.id)} className="bg-[#7c3aed] text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded flex items-center gap-2">
