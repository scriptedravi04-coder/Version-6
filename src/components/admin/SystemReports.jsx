       ...
       <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {collabs.filter(c => c.status === 'Disputed').map(c => (
                <div key={c.id} className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5 hover:border-red-500/50 transition-colors cursor-pointer group" onClick={() => { setActiveCollab(c); setActiveView('dispute_detail')}}>
                   <div className="flex justify-between items-start mb-4">
                      ...
                      <span className="text-xs text-foreground/50">{c.lastUpdate}</span>
                   </div>
                   <h3 className="font-semibold text-lg mb-1">{c.campaign}</h3>
                   <div className="text-sm text-foreground/70 mb-4">{c.brand} vs {c.creator}</div>
                   <div className="flex items-center justify-between border-t border-red-500/10 pt-4">
                      <div className="text-sm"><span className="text-foreground/50">Escrow:</span> <span className="font-mono font-medium">₹{c.escrow?.toLocaleString() || 0}</span></div>
             ...
             {collabs.filter(c => c.status === 'Disputed').length === 0 && (
                <div className="col-span-full p-6 text-center text-foreground/50 border border-dashed border-foreground/10 rounded-2xl">
                   ...
                   {collabs.map(c => (
                      <tr key={c.id} className="hover:bg-foreground/5 transition-colors">
                         <td className="px-4 py-4">
                            <div className="font-mono text-xs text-foreground/40 mb-1">{c.id}</div>
                            <div className="font-semibold">{c.campaign}</div>
                         </td>
                         <td className="px-4 py-4">
                            <div>{c.brand}</div>
                            <div className="text-foreground/50 text-xs mt-0.5">With: {c.creator}</div>
                         </td>
                         <td className="px-4 py-4">
                            <div className="mb-1">
                            ...
                            <div className="text-xs text-foreground/40">Due: {c.deadline}</div>
                         </td>
                         <td className="px-4 py-4 text-right">
                            <button 
                               onClick={() => { setActiveCollab(c); setActiveView('collab_detail'); }}
                               className="px-3 py-1.5 bg-black/40 border border-foreground/10 hover:border-[#9D7CFF]/50 hover:bg-foreground/5 rounded-lg text-xs font-semibold transition-colors"
                            >
                               Manage Collab
                            </button>
                         </td>
...
function CollabStatusControl({ collab, onBack }) {
  ...
  return (
    <div className="w-full max-w-5xl space-y-6">
       <button onClick={onBack} className="text-sm text-foreground/60 hover:text-white flex items-center gap-2 mb-4">
          &larr; Back to Reports
       </button>

       <div className="p-6 bg-card border border-foreground/10 rounded-2xl flex items-center justify-between">
          <div>
             <h2 className="font-display text-2xl font-bold">{collab.campaign}</h2>
             <div className="text-sm text-foreground/50 mt-1">{collab.brand} • {collab.creator}</div>
          </div>
          <div className="text-right">
             <div className="text-xs text-foreground/50 uppercase tracking-widest font-semibold mb-1">Escrow Funds</div>
          ...
          <div className="space-y-6">
             <div className="bg-card border border-foreground/10 rounded-2xl p-6">
                ...
                <div className="space-y-4">
                   <div>
                      <label className="text-xs font-medium text-foreground/50 block mb-1">New Status Target</label>
                      <select 
                         value={overrideStatus}
                         onChange={e=>setOverrideStatus(e.target.value)}
                         className="w-full p-3 bg-black/40 border border-foreground/10 rounded-xl focus:border-[#9D7CFF] focus:outline-none text-sm"
                      >
                   ...
                   <div>
                      <label className="text-xs font-medium text-foreground/50 block mb-1">Reason for manual change (Mandatory log)</label>
                      <textarea 
                         value={reason}
                         onChange={e=>setReason(e.target.value)}
                         className="w-full p-3 bg-black/40 border border-foreground/10 rounded-xl focus:border-[#9D7CFF] focus:outline-none text-sm h-24 resize-none"
                      />
                   </div>
                   <button 
             ...
             <div className="bg-card border border-foreground/10 rounded-2xl p-6">
                ...
                <p className="text-xs text-foreground/40 mt-3">Both parties will be notified of the deadline extension.</p>
             </div>
          </div>

          <div className="space-y-6">
             <div className="bg-card border border-foreground/10 rounded-2xl p-6 h-[400px] flex flex-col">
                ...
                <div className="flex-1 bg-black/20 rounded-xl border border-foreground/5 p-4 overflow-y-auto mb-4 italic text-sm text-foreground/50 flex flex-col justify-end">
                   [Brand and Creator conversation context history here...]
                </div>
                
                <div>
                   <select className="w-full mb-2 p-2 bg-black/40 border border-foreground/10 rounded-lg text-xs focus:border-[#9D7CFF] focus:outline-none text-foreground/70">
                   ...
                   <textarea placeholder="Type admin message..." className="w-full h-16 p-3 bg-black/40 border border-foreground/10 rounded-xl text-sm focus:border-[#9D7CFF] focus:outline-none resize-none mb-2"></textarea>
                   <button className="w-full py-2 bg-[#9D7CFF] hover:bg-[#8B6BE0] text-white font-semibold rounded-xl text-sm transition-colors text-center">
                      Send to Chat Log
                   </button>
                </div>
             </div>
  ...
  return (
     <div className="w-full max-w-7xl flex flex-col h-[calc(100vh-100px)]">
       <div className="flex items-center justify-between mb-6 shrink-0">
          <button onClick={onBack} className="text-sm text-foreground/60 hover:text-white flex items-center gap-2">
             &larr; Back to Reports
          </button>
          <div className="flex items-center gap-3">
             ...
             <div className="text-sm font-mono text-foreground/50">Escrow: <span className="text-white font-bold text-base">₹{collab.escrow?.toLocaleString() || 0}</span></div>
          </div>
       </div>

       <div className="flex flex-1 gap-6 min-h-0">
          {/* Left Panel: Brand Complaint */}
          <div className="flex-1 bg-card border border-foreground/10 rounded-2xl flex flex-col overflow-hidden">
             <div className="p-4 bg-black/20 border-b border-foreground/10">
                <span className="text-xs uppercase font-bold text-foreground/50 tracking-wider">Brand Claim</span>
             ...
             <div className="p-5 overflow-y-auto space-y-4">
                ...
                <div>
                   <div className="text-xs font-medium text-foreground/50 mb-2">Evidence Uploaded</div>
                   <div className="grid grid-cols-2 gap-2">
                      <div className="aspect-video bg-black/40 rounded-lg flex items-center justify-center border border-foreground/10 text-foreground/40 text-xs">Brief_PDF.pdf</div>
                      <div className="aspect-video bg-black/40 rounded-lg flex items-center justify-center border border-foreground/10 text-foreground/40 text-xs">Chat_Log.png</div>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="flex-1 bg-card border border-foreground/10 rounded-2xl flex flex-col overflow-hidden">
             <div className="p-4 bg-black/20 border-b border-foreground/10">
                <span className="text-xs uppercase font-bold text-foreground/50 tracking-wider">Creator Response</span>
             ...
             <div className="p-5 overflow-y-auto space-y-4">
                ...
                <div>
                   <div className="text-xs font-medium text-foreground/50 mb-2">Evidence Uploaded</div>
                   <div className="grid grid-cols-2 gap-2">
                      <div className="aspect-video bg-black/40 rounded-lg flex items-center justify-center border border-foreground/10 text-foreground/40 text-xs">Draft_Video.mp4</div>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="flex-[1.2] flex flex-col gap-4 min-h-0">
             <div className="bg-card border border-[#9D7CFF]/30 shadow-[0_0_20px_rgba(157,124,255,0.05)] rounded-2xl p-6 flex flex-col h-full overflow-y-auto">
                ...
                <div className="space-y-3 mb-8">
                   <label className={`flex p-4 border rounded-xl cursor-pointer transition-colors ${resolution === 'refund' ? 'bg-red-500/10 border-red-500' : 'border-foreground/10 hover:bg-foreground/5'}`}>
                      ...
                      <div>
                         ...
                         <div className="text-xs text-foreground/60 mt-1">Collab marked failed. Brand receives 100% back to wallet.</div>
                      </div>
                   </label>
                   
                   <label className={`flex flex-col p-4 border rounded-xl cursor-pointer transition-colors ${resolution === 'partial' ? 'bg-amber-500/10 border-amber-500' : 'border-foreground/10 hover:bg-foreground/5'}`}>
                      <div className="flex items-start">
                         ...
                         <div>
                            ...
                            <div className="text-xs text-foreground/60">Both parties receive a custom percentage of the escrowed amount.</div>
                         </div>
                      </div>
                      {resolution === 'partial' && (
                         <div className="mt-4 pt-4 border-t border-amber-500/20 pt-4 flex items-center gap-4 animate-in slide-in-from-top-2">
                            <div className="flex-1">
                               ...
                               <input type="number" defaultValue={50} className="w-full bg-black/40 border border-amber-500/30 rounded-lg p-2 text-center font-mono focus:outline-none" />
                            </div>
                            <div className="flex-1">
                               <label className="text-[10px] uppercase tracking-wider text-green-500/70 font-bold mb-1 block">Creator Payout %</label>
                               <input type="number" defaultValue={50} className="w-full bg-black/40 border border-green-500/30 rounded-lg p-2 text-center font-mono focus:outline-none" />
                            </div>
                         </div>
                      )}
                   </label>

                   <label className={`flex p-4 border rounded-xl cursor-pointer transition-colors ${resolution === 'pay' ? 'bg-green-500/10 border-green-500' : 'border-foreground/10 hover:bg-foreground/5'}`}>
                      ...
                      <div>
                         ...
                         <div className="text-xs text-foreground/60 mt-1">Collab marked complete. Creator receives full payout minus standard platform fees.</div>
                      </div>
                   </label>
                </div>

                <div className="mt-auto">
                   <label className="text-xs font-semibold text-foreground/50 uppercase tracking-wider mb-2 block">Resolution Rationale (Sent to both)</label>
                   <textarea 
                      placeholder="Explain the platform's decision..." 
                      className="w-full h-24 bg-black/40 border border-foreground/10 rounded-xl p-3 text-sm focus:outline-none focus:border-[#9D7CFF] resize-none mb-4"
                   />
                   <button 
