...
export default function FeeBreakup({ grossAmount, onDetailsCalculated }) {
  ...
  if (loading || !feeDetails) return <div className="animate-pulse h-32 bg-white/5 rounded-xl"></div>;

  return (
    <div className="bg-[#1A1A2E] border border-white/10 rounded-xl p-5 text-sm">
      <div className="flex justify-between items-center mb-3">
        <span className="text-white/60">Campaign Amount</span>
        <span className="font-bold text-white">₹{feeDetails.grossAmount.toLocaleString('en-IN')}</span>
      </div>
      <div className="flex justify-between items-center mb-3">
        <span className="text-white/60">Platform Fee ({feeDetails.feePercent}%)</span>
        <span className="text-rose-400 font-medium">-₹{feeDetails.platformFee.toLocaleString('en-IN')}</span>
      </div>
      <div className="flex justify-between items-center mb-4 pb-4 border-b border-white/5">
        <span className="text-white/60">GST on fee (18%)</span>
        <span className="text-amber-400 font-medium">+₹{feeDetails.gstAmount.toLocaleString('en-IN')}</span>
      </div>
      <div className="flex justify-between items-center">
        <span className="font-bold text-white">Creator Receives</span>
